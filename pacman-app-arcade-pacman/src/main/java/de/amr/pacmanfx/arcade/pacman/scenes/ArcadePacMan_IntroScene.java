...
public class ArcadePacMan_IntroScene extends GameScene2D {
    ...
    public void doInit(Game game) {
        ...
        actionBindings.registerAllFrom(ArcadePacMan_UIConfig.GAME_START_BINDINGS); // insert coin + start game actions
        actionBindings.registerAllFrom(GameUI.SCENE_TESTS_BINDINGS); // actions for starting tests
            ...
            public void onUpdate(ArcadePacMan_IntroScene scene) {
                ...
                if (timer.atSecond(0.75)) {
                    ...
                    if (!game.canStartNewGame()) {
                        game.control().stateMachine().enterState(GameState.STARTING_GAME_OR_LEVEL);
                    }
                } else if (timer.atSecond(5)) {
                    game.control().stateMachine().enterState(GameState.SETTING_OPTIONS_FOR_START);
                }
            }
        };
