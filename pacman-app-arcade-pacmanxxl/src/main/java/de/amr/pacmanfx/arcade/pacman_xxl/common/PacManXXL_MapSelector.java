...
public class PacManXXL_MapSelector implements WorldMapSelector, PathWatchEventListener {
    ...
    public void handleWatchEvents(List<WatchEvent<Path>> watchEvents) {
        ...
        for (WatchEvent<Path> event : watchEvents) {
            ...
            final File file = GameGlobals.CUSTOM_MAP_DIR.toPath().resolve(relPath).toFile();
    ...
    public void loadCustomMaps() {
        ...
        final File[] worldMapFiles = GameGlobals.CUSTOM_MAP_DIR.listFiles((_, name) -> name.endsWith(".world"));
        if (worldMapFiles == null) {
            Logger.error("Could not access custom map directory '{}'", GameGlobals.CUSTOM_MAP_DIR);
            return;
        }
        if (worldMapFiles.length == 0) {
            Logger.info("No custom maps found in directory '{}'", GameGlobals.CUSTOM_MAP_DIR);
        } else {
    ...
    private void addJuniorPacMapPrototypesIfEmptyDir() {
        final File[] files = GameGlobals.CUSTOM_MAP_DIR.listFiles();
        if (files == null) {
            Logger.error("Could not access custom map directory '{}'", GameGlobals.CUSTOM_MAP_DIR);
            return;
        }
        if (files.length == 0) {
            ...
            for (int i = 1; i <= 15; ++i) {
                ...
                if (url != null) {
                    final File targetFile = new File(GameGlobals.CUSTOM_MAP_DIR, mapName);
