...
public class PacManXXL_StartPage implements StartPage {
    ...
    private class MenuBinding {
        ...
        public MenuBinding(AppContext context) {
            gameVariantNameListener = (_, _, variant) -> context.currentGameContext().selectGameVariant(variant.name());
    ...
    public void onEnterStartPage(AppContext context) {
        ...
        switch (selectedGameVariant) {
            case ARCADE_PACMAN_XXL,ARCADE_MS_PACMAN_XXL -> context.currentGameContext().selectGameVariant(selectedGameVariant.name());
