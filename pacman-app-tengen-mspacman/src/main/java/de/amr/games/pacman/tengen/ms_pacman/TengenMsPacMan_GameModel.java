...
public class TengenMsPacMan_GameModel extends GameModel {
    ...
    private class TengenActorSpeedControl implements ActorSpeedControl {

        private float speedUnitsToPixels(float units) { return units / 32f; }

        private float pacBoosterSpeedDelta() { return 0.5f; }

        private float pacDifficultySpeedDelta(Difficulty difficulty) {
        ...
        private float ghostDifficultySpeedDelta(Difficulty difficulty) {
            return speedUnitsToPixels(switch (difficulty) {
                case EASY -> -8;
                case NORMAL -> 0;
                case HARD -> 16;
        ...
        private float ghostIDSpeedDelta(byte ghostID) {
         ...
         * </p>
         */
        private float ghostSpeedIncreaseByFoodRemaining() {
            byte units = 0;
        ...
        private float pacBaseSpeedInLevel(int levelNumber) {
            int units = 0;
        ...
        private float ghostBaseSpeedInLevel(int levelNumber) {
        ...
        public float ghostAttackSpeed(GameLevel level, Ghost ghost) {
            if (level.isTunnel(ghost.tile())) {
                return speedControl.ghostTunnelSpeed(level, ghost);
            }
            float speed = ghost.baseSpeed();
            float increase = ghostSpeedIncreaseByFoodRemaining();
            if (increase > 0) {
                speed += increase;
        ...
        @Override
        public float ghostSpeedInsideHouse(GameLevel level, Ghost ghost) { return 0.5f; }

        @Override
        public float ghostSpeedReturningToHouse(GameLevel level, Ghost ghost) { return 2; }

        @Override
        public float ghostFrightenedSpeed(GameLevel level, Ghost ghost) { return 0.5f * ghost.baseSpeed(); } //TODO is this correct?

        @Override
        public float ghostTunnelSpeed(GameLevel level, Ghost ghost) { return 0.4f * ghost.baseSpeed(); } //TODO is this correct?
    }
