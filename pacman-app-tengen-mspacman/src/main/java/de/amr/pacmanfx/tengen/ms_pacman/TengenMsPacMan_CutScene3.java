...
import static de.amr.pacmanfx.tengen.ms_pacman.TengenMsPacMan_SpriteSheet.SpriteID.STORK;
...
public class TengenMsPacMan_CutScene3 extends GameScene2D {
    ...
    public void update() {
        ...
        if (t == 0) {
            ...
            stork.show();
            storkAnimation = createAnimation().sprites(getSprites(STORK)).frameTicks(8).endless();
            storkAnimation.play();
            bagReleased = false;
        }
        else if (t == 270) {
    ...
    public void drawSceneContent() {
        ...
        r.drawClapperBoard(clapAnimation, "JUNIOR", 3, CLAP_TILE_X, CLAP_TILE_Y, normalArcadeFont());
        r.drawStork(storkAnimation, stork, bagReleased);
        r.drawActor(msPacMan);
        r.drawActor(pacMan);
        if (bagWithJunior.isVisible()) {
            if (bagOpen) {
                r.drawActorSprite(bagWithJunior, getSprite(SpriteID.JUNIOR_PAC));
            } else {
                r.drawActorSprite(bagWithJunior, getSprite(SpriteID.BLUE_BAG));
            }
        }
