...
public class TengenMsPacMan_OptionsScene extends GameScene2D {
    ...
    protected void drawSceneContent() {
        ...
        r.fillText("MS PAC-MAN OPTIONS", NES_YELLOW, normalFont(), COL_LABEL + 3 * TS, 48);

        // Players (not implemented)
        drawArrowIfSelected(OPTION_PLAYERS, 72, normalFont());
        r.fillText("TYPE", NES_YELLOW, normalFont(), COL_LABEL, 72);
        r.fillText(":", NES_YELLOW, normalFont(), COL_LABEL + 4 * TS + 4, 72);
        r.fillText("1 PLAYER", NES_WHITE, normalFont(), COL_LABEL + 6 * TS  , 72);

        // Pac-Booster
        drawArrowIfSelected(OPTION_PAC_BOOSTER, 96, normalFont());
        r.fillText("PAC BOOSTER", NES_YELLOW, normalFont(), COL_LABEL, 96);
        r.fillText(":", NES_YELLOW, normalFont(), COL_COLON, 96);
        String pacBoosterText = switch (tengenGame.pacBooster()) {
            case OFF -> "OFF";
            case ALWAYS_ON -> "ALWAYS ON";
            case USE_A_OR_B -> "USE A OR B";
        };
        r.fillText(pacBoosterText, NES_WHITE, normalFont(), COL_VALUE, 96);

        // Game difficulty
        drawArrowIfSelected(OPTION_DIFFICULTY, 120, normalFont());
        r.fillText("GAME DIFFICULTY", NES_YELLOW, normalFont(), COL_LABEL, 120);
        r.fillText(":", NES_YELLOW, normalFont(), COL_COLON, 120);
        r.fillText(tengenGame.difficulty().name(), NES_WHITE, normalFont(), COL_VALUE, 120);

        // Maze (type) selection
        drawArrowIfSelected(OPTION_MAZE_SELECTION, 144, normalFont());
        r.fillText("MAZE SELECTION", NES_YELLOW, normalFont(), COL_LABEL, 144);
        r.fillText(":", NES_YELLOW, normalFont(), COL_COLON, 144);
        r.fillText(tengenGame.mapCategory().name(), NES_WHITE, normalFont(), COL_VALUE, 144);

        // Starting level number
        drawArrowIfSelected(OPTION_STARTING_LEVEL, 168, normalFont());
        r.fillText("STARTING LEVEL", NES_YELLOW, normalFont(), COL_LABEL, 168);
        r.fillText(":", NES_YELLOW, normalFont(), COL_COLON, 168);
        r.fillText(String.valueOf(tengenGame.startLevelNumber()), NES_WHITE, normalFont(), COL_VALUE, 168);
        if (tengenGame.numContinues() < 4) {
            r.drawSpriteScaled(CONTINUES_SPRITES[tengenGame.numContinues()], COL_VALUE + 3 * TS, 160);
        }

        r.fillText("MOVE ARROW WITH JOYPAD", NES_YELLOW, normalFont(), 4 * TS,  192);
        r.fillText("CHOOSE OPTIONS WITH A AND B", NES_YELLOW, normalFont(), 2 * TS,  200);
        r.fillText("PRESS START TO START GAME", NES_YELLOW, normalFont(), 3 * TS,  208);
    }
