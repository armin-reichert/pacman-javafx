...
public class TengenMsPacMan_GhostAnimationManager extends SpriteAnimationManager<SpriteID> {
    ...
    protected SpriteAnimation createAnimation(String id) {
        return switch (id) {
            ...
            case ANIM_GHOST_FRIGHTENED  -> SpriteAnimation.build().of(spriteSheet().spriteSequence(SpriteID.GHOST_FRIGHTENED)).frameTicks(FRIGHTENED_TICKS).forever();
            case ANIM_GHOST_FLASHING    -> SpriteAnimation.build().of(spriteSheet().spriteSequence(SpriteID.GHOST_FLASHING)).frameTicks(FLASH_TICKS).forever();
