...
public class TengenMsPacMan_PlayScene2D extends GameScene2D implements SubSceneProvider {
    ...
    private void updateSound(GameLevel gameLevel) {
        ...
        final boolean pacChased = context().currentGameState() == TengenMsPacMan_GameState.HUNTING && !pac.powerTimer().isRunning();
        if (pacChased) {
            selectAndPlaySiren();
        }
    ...
    private void updateHUD(GameLevel gameLevel) {
        ...
        if (context().currentGameState() == TengenMsPacMan_GameState.STARTING_GAME_OR_LEVEL && !gameLevel.pac().isVisible()) {
            numLives += 1;
        }
