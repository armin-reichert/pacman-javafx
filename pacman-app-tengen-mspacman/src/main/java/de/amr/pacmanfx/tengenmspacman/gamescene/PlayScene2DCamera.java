...
public class PlayScene2DCamera extends ParallelCamera {
    ...
    public void updateRange(TerrainLayer terrain) {
        final int mapHeightTiles = terrain.numRows();
    ...
    public void update(double mapHeightPixels, GameEntity movingActor) {
        switch (state) {
    ...
    private void updateTrackingMode(double mapHeightPixels, GameEntity movingActor) {
        final WorldNavigationComp worldNavigation = movingActor.reqComp(WorldNavigationComp.class);

        double relY = movingActor.pos().y() / mapHeightPixels;
        if (relY < 0.5 || relY < 0.6 && worldNavigation.moveDir() == Direction.UP) {
            setTargetToTop();
        } else if (relY > 0.5 || relY > 0.4 && worldNavigation.moveDir() == Direction.DOWN) {
            setTargetToBottom();
        }
        move();
    }
