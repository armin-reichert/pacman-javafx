...
import static de.amr.pacmanfx.tengenmspacman.config.TengenMsPacManGameVariant.bonusValueSpriteIndex;
...
public class TengenMsPacMan_ActorRenderer extends BaseRenderer implements SpriteRendererMixin, ActorRenderer {
    ...
    private FacingSprite computePacSprite(Pac pac) {
        final SpriteAnimationAccessor animationManager = pac.animations();
    ...
    private RectShort computeBonusSprite(Bonus bonus) {
        return switch (bonus.state()) {
            ...
            case EATEN -> spriteOrDefault(spriteSheet().sprites(SpriteID.BONUS_VALUES), bonusValueSpriteIndex(bonus.symbolCode()));
