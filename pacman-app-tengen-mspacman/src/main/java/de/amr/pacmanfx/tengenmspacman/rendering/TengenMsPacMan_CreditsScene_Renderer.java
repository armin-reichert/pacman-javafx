...
public class TengenMsPacMan_CreditsScene_Renderer extends BaseRenderer implements GameScene2D_Renderer, TengenMsPacMan_SceneRendererMixin {
    ...
    public void draw(GameScene2D scene) {
        clearCanvas();
        drawHorizontalBar(nesColor(0x20), nesColor(0x13), scene.getUnscaledWidth(), TS, 20);
        drawHorizontalBar(nesColor(0x20), nesColor(0x13), scene.getUnscaledWidth(), TS, 212);
        ctx.setFont(arcadeFont8());
        y = 7 * TS; // important: reset on every draw!
        if (scene.context().currentGameContext().currentGameState().timer().betweenSeconds(0, 0.5 * DISPLAY_SECONDS)) {
