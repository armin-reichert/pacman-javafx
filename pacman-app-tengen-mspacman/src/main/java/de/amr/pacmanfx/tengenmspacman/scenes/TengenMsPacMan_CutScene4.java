...
public class TengenMsPacMan_CutScene4 extends GameScene2D {
    ...
    public TengenMsPacMan_CutScene4(AppContext context) {
        super(context);
    ...
    public void onDeactivate() {
        context.ui().sounds().stop(PacManGameSoundID.INTERMISSION_4);
    }
    ...
    public void onTick(GameClock clock) {
        ...
        if (gameStateTick <= TICK_EXPIRES) {
            ...
            switch (eventTick) {
                ...
                case 1500 -> context().currentSoundEffects().ifPresent(GameSoundEffects::stopAll);
                case TICK_EXPIRES -> game.flow().enterState(TengenMsPacMan_GameState.PREPARING_GAME_START.state());
            }
        }
    }
    ...
    private void spawnJunior(UIConfig uiConfig, long tick) {
        ...
        junior.setSpeed(2);
        junior.setAnimations(uiConfig.createPacAnimations(context.ui().sprites().animationSet()));
        ...
        context.ui().sounds().loop(soundID);
