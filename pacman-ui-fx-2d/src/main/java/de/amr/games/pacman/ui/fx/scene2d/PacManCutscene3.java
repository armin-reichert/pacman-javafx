...
public class PacManCutscene3 extends GameScene2D {
	...
	public void init() {
		...
		blinky.setAnimations(new GhostSpriteAnimationsPacManGame(blinky, context.ui().spritesheetPacManGame()));
	...
	public void update() {
		if (initialDelay > 0) {
			--initialDelay;
			if (initialDelay == 0) {
				GameEvents.publishSoundEvent(GameModel.SE_START_INTERMISSION_3, context.game());
			}
			return;
		}
