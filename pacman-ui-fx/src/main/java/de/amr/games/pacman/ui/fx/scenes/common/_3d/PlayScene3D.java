...
public class PlayScene3D implements GameScene {
	...
	final GameRendering2D rendering2D = gameVariant == GameVariant.MS_PACMAN ? rendering2D_MsPacMan
		: rendering2D_PacMan;
	...
	ghosts3D = game().ghosts()
		.collect(Collectors.toMap(Function.identity(), ghost -> new Ghost3D(ghost, rendering2D)));

	bonus3D = new Bonus3D(gameVariant, rendering2D);
