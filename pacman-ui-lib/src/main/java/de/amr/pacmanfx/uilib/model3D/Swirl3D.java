...
public class Swirl3D extends Group implements Disposable {
    ...
    public Swirl3D(AnimationRegistry animationRegistry, String label, float rotationSeconds) {
        ...
        animation.setFactory(() -> new ParallelTransition(
            createSwirlRotation(rotationSeconds),
            createWrappedParticleMovement()
        ));
    }

    private RotateTransition createSwirlRotation(float rotationSeconds) {
    ...
    private Transition createWrappedParticleMovement() {
        final var transition = new Transition() {
            @Override
