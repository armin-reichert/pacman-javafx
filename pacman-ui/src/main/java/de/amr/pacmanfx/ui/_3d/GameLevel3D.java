...
public class GameLevel3D implements Disposable {
    ...
    private void createEnergizers3D() {
        ...
        Material[] ghostDressMaterials = {
            ghosts3D.get(RED_GHOST_SHADOW).ghost3D().dressMaterial(),
            ghosts3D.get(PINK_GHOST_SPEEDY).ghost3D().dressMaterial(),
            ghosts3D.get(CYAN_GHOST_BASHFUL).ghost3D().dressMaterial(),
            ghosts3D.get(ORANGE_GHOST_POKEY).ghost3D().dressMaterial(),
        };
    ...
    private void consumeCollectedParticlesInsideHouse(Ghost ghost, Group particlesGroup) {
        ...
        for (Node child : particlesGroup.getChildren()) {
            if (child instanceof Explosion.Particle particle) {
                ...
                Material ghostDressMaterial = ghosts3D.get(ghost.personality()).ghost3D().dressMaterial();
