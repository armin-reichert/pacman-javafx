...
public class GameLevel3D extends Group implements Disposable {
    ...
    private void updateLivesCounter3D() {
        if (livesCounter3D != null) {
            ...
            boolean oneMore = ui.context().currentGameState() == GamePlayState.STARTING_GAME_OR_LEVEL && !gameLevel.pac().isVisible();
    ...
    public void onPacManDying(FsmState<GameContext> state) {
        ...
        animation.setOnFinished(e -> ui.context().currentGame().stateMachine().letCurrentStateExpire());
        animation.play();
    }

    public void onGhostDying() {
        gameLevel.game().simulationStepResults().killedGhosts.forEach(killedGhost -> {
