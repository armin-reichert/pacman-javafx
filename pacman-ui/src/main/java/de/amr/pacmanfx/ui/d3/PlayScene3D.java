...
import static de.amr.pacmanfx.ui.GameUI_Constants.PROPERTY_3D_DRAW_MODE;
import static de.amr.pacmanfx.ui.GameUI_Constants.PROPERTY_3D_LIGHT_COLOR;
...
public class PlayScene3D extends GameScene implements DisposableGraphicsObject {
    ...
    public PlayScene3D(GameUI ui) {
        super(ui);
        gameOverMessagePicker = new RandomTextPicker(ui.facade().translations().bundle(), "game.over");
        createSubScene();
        createBindings();
        bindActions();
    ...
    public void updateHUD3D(GameLevel level) {
        ...
        if (score.isEnabled()) {
            ...
            scores3D.showTextForScore(ui.facade().translations().translate("score.game_over"),
    ...
    public void replaceGameLevel3D(GameLevel level) {
        ...
        level3D.createAnimations(GameUI_Constants.DEFAULT_PARTICLE_ANIMATION_CONFIG);

        Logger.info("Created and added new 3D game level to play scene");
    }

    public void showRandomGameOverMessage() {
        ui.facade().showFlashMessage(Duration.seconds(2.5), gameOverMessagePicker.selectNextText());
    }
    ...
    public void onActivate(UIConfig uiConfig) {
        perspectives.activeIDProperty().bind(GameUI_Constants.PROPERTY_3D_PERSPECTIVE_ID);
    ...
    public void onTick(GameClock clock) {
        final GameLevel level = ui.facade().currentGame().optGameLevel().orElse(null);
        if (level == null) {
        ...
        ui.facade().currentSoundEffects().ifPresent(soundEffects -> {
    ...
    private void createSubScene() {
        ...
        coordinateSystem.visibleProperty().bind(GameUI_Constants.PROPERTY_3D_AXES_VISIBLE);
    ...
    private void replaceScores3D() {
        ...
        scores3D = new Scores3D(
            ui.facade().translations().translate("score.score"),
            ui.facade().translations().translate("score.high_score"),
