...
public class PlayScene3DGameEventHandler extends GameScene.DefaultGameEventHandler {
    ...
    public void onPacGetsPower(PacGetsPowerEvent event) {
        ...
        if (!context().currentGame().isLevelCompleted(level3D.level())) {
    ...
    private void playLevelEndAnimation(AnimationRegistry animationRegistry, Maze3D maze3D, State<GameModel> gameState, boolean cutSceneAfter) {
        ...
        final Animation restoreCameraPerspective = Ufx.pauseSecThen(0.25, () -> {
            AppConstants.PROPERTY_3D_PERSPECTIVE_ID.set(perspectiveBeforeAnimation);
            maze3D.wallBaseHeightProperty().bind(AppConstants.PROPERTY_3D_WALL_HEIGHT);
        });
