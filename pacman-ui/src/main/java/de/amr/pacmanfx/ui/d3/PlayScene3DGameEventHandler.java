...
public class PlayScene3DGameEventHandler extends GameScene.DefaultGameEventHandler {
    ...
    public void onPacEatsFood(PacEatsFoodEvent event) {
        ...
        if (event.allPellets()) {
            ...
            if (event.energizer()) {
                ...
                gameScene().optSoundEffects().ifPresent(GameSoundEffects::playEnergizerExplosion);
            }
            else {
    ...
    public void onPacGetsPower(PacGetsPowerEvent event) {
        final GameLevel3D level3D = assertLevel3D();
        gameScene().optSoundEffects().ifPresent(GameSoundEffects::stopSiren);
    ...
    private Animation createPacDyingAnimationSeq(AnimationRegistry animationRegistry, Pac3D pac3D, GameLevel level) {
        ...
        return new SequentialTransition(
            pacStopping,
            Ufx.pauseSecThen(1.5, () -> gameScene().optSoundEffects().ifPresent(GameSoundEffects::playPacDeadSound)),
            pacDying,
            Ufx.pauseSec(0.5)
        );
    }
