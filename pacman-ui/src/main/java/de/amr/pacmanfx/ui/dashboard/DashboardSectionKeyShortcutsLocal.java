...
public class DashboardSectionKeyShortcutsLocal extends DashboardSection {
    ...
    public void update(GameUI ui) {
        clearGrid();
        ui.management().gameSceneManager().optCurrentGameScene().ifPresent(gameScene -> addEntries(ui,gameScene));
    }
        ...
        if (actionBindings.bindingMap().isEmpty()) {
            ...
            actionBindings.bindingMap().entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    final String localizedActionText = ui.management().translationManager().translate(action.resourceBundleKey());
