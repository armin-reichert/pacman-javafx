...
public class DashboardSectionKeyShortcutsLocal extends DashboardSection {
    ...
    public void update(GameUI ui) {
        clearGrid();
        ui.facade().gameScenes().optCurrentGameScene().ifPresent(gameScene -> addEntries(ui,gameScene));
    }
        ...
        if (actionBindings.bindingMap().isEmpty()) {
            ...
            actionBindings.bindingMap().entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    final String localizedActionText = ui.facade().translations().translate(action.resourceBundleKey());
