...
public class PlayScene3D extends AbstractGameScene implements DisposableGraphicsObject {
    ...
    public void updateHUD3D(GameLevel level) {
        ...
        if (score.isEnabled()) {
            ...
            scores3D.showTextForScore(
                ...
                game().variant().assets().color("color.game_over_message"));
        }
    ...
    public void replaceGameLevel3D(GameLevel level) {
        ...
        level3D = new GameLevel3D(viewModel, gameContext(), level, game().variant());
        decorate(level3D);
    ...
    public void onTick(long tick) {
        final GameLevel level = gameContext().model().optGameLevel().orElse(null);
