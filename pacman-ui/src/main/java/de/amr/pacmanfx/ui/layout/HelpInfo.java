...
public class HelpInfo {
    ...
    public static HelpInfo build(GameUI ui) {
        ...
        if (state.matchesByName(CanonicalGameState.INTRO.name())) {
            ...
            if (demoLevel) {
                ...
                helpInfo.addInfoForPlayScene(ui.facade().translations());
            }
        }
        else {
    ...
    public Pane createPane(GameUI ui, Color backgroundColor, Font font) {
        ...
        // add default entries:
        if (ui.facade().currentGame().cheats().isUsingAutopilot()) {
            var autoPilotEntry = text(ui.facade().translations().translate("help.autopilot_on"), Color.ORANGE);
            autoPilotEntry.setFont(font);
            GridPane.setColumnSpan(autoPilotEntry, 2);
            grid.add(autoPilotEntry, 0, grid.getRowCount());
        }
        if (ui.facade().currentGame().cheats().isImmune()) {
            var immunityEntry = text(ui.facade().translations().translate("help.immunity_on"), Color.ORANGE);
    ...
    private void addRow(String lhsKey, String keyboardKey) {
        addRow(label(ui.facade().translations().translate(lhsKey), Color.gray(0.9)),
