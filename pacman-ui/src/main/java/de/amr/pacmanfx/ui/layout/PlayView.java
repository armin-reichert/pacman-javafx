...
public class PlayView extends StackPane implements View {
    ...
    public void onGameEvent(GameEvent gameEvent) {
        ...
        switch (gameEvent) {
            ...
            case GameStateChangeEvent _ -> {
                if (gameState.nameMatches(GameControl.CommonStateName.LEVEL_COMPLETE.name())) {
                    miniView.slideOut();
                }
            }
            default -> {}
    ...
    private void useDecoratedCanvas(GameUI ui, GameScene2D gameScene2D) {
        ...
        sceneRenderer = ui.currentConfig().createGameSceneRenderer(ui, canvas, gameScene2D);
    ...
    private void embedGameScene(Scene parentScene, GameScene gameScene) {
        ...
        if (gameScene.optSubScene().isPresent()) {
            ...
            if (ui.currentGameSceneConfig().sceneDecorationRequested(gameScene)) {
                final float maxScaling = ui.prefs().getFloat("scene2d.max_scaling");
